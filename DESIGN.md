...
- **Onion service key** (Ed25519 v3) — the hidden service this user publishes. **⚠ IMPLEMENTATION STATUS (A6.4 / D-4): NOT yet derived from the signing key.** The *intent* is to derive the onion key deterministically from the identity signing key so the user's fingerprint and their `.onion` address are mathematically equivalent — letting a peer verify "this onion belongs to this fingerprint" with no extra trust. **That property is not implemented today.** The current code (`crates/onyx-core/src/tor.rs::publish_hidden_service`) lets Arti generate a random, keystore-managed HS identity via `OnionServiceConfigBuilder::default()`, so today the `.onion` and the fingerprint are **independent keys with no cryptographic link**. **Do NOT rely on onion↔fingerprint equivalence for authentication** until this lands — verify peers by fingerprint (which the sealed-sender/MLS layers actually authenticate), not by onion address. Closing it needs an arti `KeyMgr` importer that builds an `HsIdKeypair` from the identity key and installs it before `launch_onion_service`; tracked as A6.4. When implemented, rotating identity will also rotate the onion (breaking inbound connections from contacts who haven't been re-told the new address) — which is why it's a deliberate, separately-shipped change.
