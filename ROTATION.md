...
- **2026-06-02 (fortification F1.4 review)**: the fortification plan listed "connect-time fingerprint → session-resume routing-id rotation" as a Phase-1 item. **Reviewed and deliberately NOT shipped as a rotation feature**, because this document already proves rotation *alone* is a non-fix (§2): it relocates the leak rather than closing it, and would add lockstep sender/recipient epoch coordination whose desync risk is real delivery breakage for no privacy gain.
  - **Leak #1 (Noise identity) is already closed by the private default (D-1).** Since `--first-contact-reachable` defaults OFF, the daemon already uses a *fresh per-connection ephemeral Noise static* + ephemeral SUBSCRIBE-signing key and skips the intro-inbox subscription and KP publish — exactly §3.A. So in the default posture the hub authenticates "some ephemeral peer," not alice. That is the change rotation was supposed to approximate, done properly.
  - **The genuine residual is §3.B + §3.C** — separated publish/subscribe connections and oblivious-recipient routing for first-contact. Those are tracked as fortification **Phase 2 (F2.1)**, not Phase 1, because they are the architectural work this document identifies (not a rotation tweak).
  - **Net**: F1.4 is resolved by analysis — the identity-linking vector it targeted is closed in the default config (D-1); the remaining unlinkability work is correctly scoped to Phase 2. No code change; shipping a rotation feature here would have been a false-assurance non-fix.
