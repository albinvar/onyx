...
14. **TUI must visually distinguish hub-relayed messages from direct-MLS** (affects user understanding of the threat model). T5.2.c added `BootstrapPayload::PlainMessage` (`v: msg/v1`) which has per-message PFS but no PCS. Future `mls/v1` will have both. The user cannot make the right trust decision unless the TUI labels each conversation/message with its security tier. Tracked for T5.2.f.
