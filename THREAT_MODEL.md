...
| T-2 | HIGH | Invite has no signature/MAC/nonce/expiry → invite-channel MITM owns first contact | **Fixed (partial closure of an intrinsic gap).** New `onyx://invite/v2?…` format adds `exp` + `nonce` + `sig`: the daemon signs a length-prefixed canonical blob (`SIGN_CONTEXT_V2 ‖ fp ‖ kem ‖ kp ‖ hubs ‖ exp ‖ nonce`) with the identity Ed25519 key (via the new `BuildInvite` API). Parse-side: signed invites get `verify_signature(now_ms)` called on accept; unsigned v1 still parses for back-compat and the CLI prints a loud unverified-warning. v1 paths carrying sig fields are refused as a downgrade attempt. Default TTL 30 days. **Honest scope:** this closes **partial** tampering on the side-channel (swapped KEM/KP/hubs); it does NOT defeat **full-invite substitution** — a MITM who replaces the entire invite (their own fp + their own keys + their own signature) is still indistinguishable on the channel. OOB fingerprint verification + T-1 pinning are the user-level / detect-later defences. 7 new tests cover round-trip, expiry, tampered-kem, tampered-hubs, swapped-fp, downgrade refusal, malformed sig. |
...
**Update:** D-2, T-1, T-2, and the D-3/T-3 follow-up close are all implemented. P-1 was verified closed by composition of HIGH-2 + T-1 + P-2 (see its row — analysis only, no code change needed). The only HIGH residual is now **D-1** (ephemeral Noise keys — the §3.2 redesign).
