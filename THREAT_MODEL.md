...
**Update:** D-2 (Tor circuit isolation) was implemented immediately after this pass (see the row above and the CHANGELOG). The remaining HIGH residuals are D-1 (ephemeral Noise keys), T-1 (key pinning + `Contact`), T-2 (invite auth), and P-1 (conversation keying — verification-first).
