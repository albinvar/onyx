...
| §4 trust assumption — Reproducible builds                             | ◑ | ◑ | local double-build byte-match | **Partially established (F0.1).** `release.yml` already builds deterministically (`SOURCE_DATE_EPOCH` pinned, `--remap-path-prefix`, `--locked`, symbol-strip, env version-stamp). Verified locally: two independent `--release --locked` builds of the same source with these flags produce a **byte-identical** `onyx` binary (sha256 match). `scripts/verify-reproducible-build.sh` lets anyone rebuild a tag and diff against the published `SHA256SUMS`. **Still owed:** an independent second-builder confirming the *published* release artifacts reproduce (cross-machine), and signed releases (separately tracked). |
...
|----|-----|---------|------------------------|
| D-1 | HIGH | Hub Noise static key = long-term X25519 identity → hub clusters all of a user's routing ids/tokens under one authenticated identity | **Fixed, private-by-default (re-audit follow-up).** The earlier opt-in `--ephemeral-noise-static` was theatre, correctly flagged by re-audit: it was off by default, and even when on, the SUBSCRIBE proof still shipped the long-term Ed25519 signing key (= fingerprint) + the daemon still subscribed to `introduction_inbox(fp)` + published a KeyPackage — so the hub re-linked the user on every connection regardless. Replaced with a single master switch **`first_contact_reachable` (default `false` = private)** that coherently closes **all three** §3.2 identity leaks at once: in the default mode the daemon (a) uses a fresh per-connection **ephemeral Noise static**, (b) signs the SUBSCRIBE proof with a fresh per-connection **ephemeral Ed25519 key** (NEW — this was the leak the opt-in missed), (c) does **not** subscribe to `introduction_inbox(fp)`, and (d) does **not** publish a KeyPackage. The hub therefore cannot link the connection to the long-term identity at all. Existing rooms still work (per-(room, epoch) session tokens carry no identity) and direct onion dials still work; the only thing the user gives up is being reachable for *first contact* via that hub, which they opt back into with `--first-contact-reachable`. The long-term identity is always used by the end-to-end sealed-sender layer (HIGH-2) inside Noise frames, so peer authentication is unaffected. **Adversarially tested**: `rooms_e2e_private_mode_leaks_no_identity_to_hub` asserts the hub holds **zero KeyPackages** and does not know the daemon's intro inbox in default mode, with a reachable-mode counter-check that a KP *does* land otherwise. **Residual trade**: the hub's per-static-key rate limiter (HIGH-3) is per-connection in private mode (reconnect → fresh bucket); per-frame caps still bound resource use. **Future**: separated publish/subscribe + oblivious-recipient routing would restore first-contact reachability without identity linkage even in private mode. |
...
| D-4 / A6.4 | MED | Onion service key random (`OnionServiceConfigBuilder::default()`), not identity-derived → the documented "onion address == fingerprint" equivalence is FALSE | **Overclaim corrected (code path still open).** The dangerous part was a *false security claim*: DESIGN.md §4.1 stated the onion and fingerprint are "mathematically equivalent," which a user could have relied on to authenticate a peer by onion address. That is now corrected everywhere (DESIGN §4.1, ANONYMITY §2, and the `tor.rs::publish_hidden_service` doc-comment) with an explicit "do NOT authenticate by onion; use the fingerprint" warning. **Still open (the feature):** actually deriving the HS key from the identity via an arti `KeyMgr` `HsIdKeypair` importer, which would make the equivalence true and verifiable. Tracked as A6.4. Until then the honest posture is: onion and fingerprint are independent keys; only the fingerprint is authenticated (by the sealed-sender/MLS layers). |
| T-1 | HIGH | No key pinning / "key changed" warning / `Contact` command (TOFU) | **Fixed** — the vault now pins each peer's Ed25519-fingerprint → X25519-identity-key binding on first contact (`pinned_keys` table; `Vault::pin_or_verify`). At every conversation-registration point (`pin_check_peer`: direct dial, msg/v1 bootstrap, and MLS Welcome — placed before the room/DM branch so both are covered) a later key that differs from the pinned one is **kept-not-overwritten**, flagged (`key_changed`), and `warn!`'d as a possible MITM/rotation. Surfaced via `onyx contact list` (API `ListContacts`/`ContactInfo`). **A0.3 now BLOCKS** outbound sends to a key-changed contact on every path (`Send`/`SendRoom`/`SendFileToPeer`/`SendFileToRoom`, via `api_server::pin_block` over `Vault::is_pin_compromised`); room paths fail closed if *any* member is compromised. (Honest history: the original A0.3 commit `0590c1f` wired only the two DM paths — the two room paths were added later when a clean-room audit caught that the "all four" claim was false; both room handlers now carry the `pin_block` loop.) **Residual:** no re-pin/accept UI yet (clearing a changed pin is manual); the `is_pin_compromised` predicate is unit-tested but the four per-handler call sites are verified by code-read + compile, not yet by an integration test (a pin-injection harness is owed — that missing test is exactly what let the room gap ship unnoticed); first-contact MITM itself is still T-2. |
...
| P-3 | MED | Hub deliveries processed inline + serial → junk-envelope CPU / head-of-line DoS | **Resolved (F3.1).** Inbound DELIVER processing (sealed-envelope KEM decap + MLS) moved OFF the session read loop into a single per-daemon **bounded** delivery worker; the read loop now only does a non-blocking enqueue, so a junk-envelope flood no longer stalls a session's outbound sends + keepalive. Bounded queue sheds under flood (hub retains envelopes; replay guard dedups re-delivery). Still blunted additionally by the replay guard + sealed-envelope drop. |
| G-2 | MED | No committer-authority model — any MLS member can add/remove any member | **Mitigated (F3.3).** App-layer admin model: each room has an **admin set** (seeded with the creator) propagated to every member in the Welcome. Honest clients enforce it on both sides — **send-side** refusal (a non-admin's invite/remove is refused) and **receive-side** rejection (a membership commit from a non-admin is *not merged*, so honest members ignore it). **Inherent residual** (documented, `ROOM-AUTHORITY.md`): MLS has no RBAC, so a patched client can still emit a valid commit and **fork** honest members (they reject + warn but do not auto-recover); the authenticated-admin-set (GroupContext extension) and fork-recovery variants stay deferred. Empty admin set (legacy rooms) = unrestricted, for back-compat. |
| H-1 | 🟠→🟡 | Federated peer can fill a target inbox's offline queue to the cap → targeted denial-of-delivery | **Further mitigated (F3.2a).** On top of A-1 (fill-rate limit) + the per-id depth cap, the global byte cap now uses **fair eviction**: under memory pressure the hub evicts the oldest entry from the *largest* queue instead of dropping the newest arrival, so a flood concentrated in one/few queues (the many-inbox global-exhaustion attack) can't starve the many small legitimate queues. Residual: single-inbox flood of a *reachable* user's intro inbox stays possible within the per-id cap — the private default (no targetable intro inbox) avoids it, and proof-of-work is a deferred opt-in hub policy. See `QUEUE-HARDENING.md`. **Fundamental tension** (hub can't distinguish junk from real sealed envelopes) is inherent. |
| H-2 | MED | Forgeable gossip `seen_by` → a peer can forge our hash to make us drop a frame (directory-partition / suppression) | **Fixed** — the gossip handlers (`handle_gossip_publish` / `handle_gossip_deliver`) no longer drop on the self-asserted `seen_by` at all. That field is attacker-chosen and gating a *drop* on it was the suppression vector; it also never caught a real loop, since the genuine immediate forwarder is the Noise-authenticated `source_pubkey` (always a remote peer → could never equal our own hash). Loop bounding is the TTL (depth ≤ `GOSSIP_TTL_DEFAULT`) + the source-skip on re-fanout, neither of which trusts `seen_by`. Regression test `gossip_publish_forged_self_seen_by_is_not_dropped` asserts a frame with `seen_by = hash(us)` is now accepted + re-fanned-out. **Residual (cleanliness, not security):** the now-inert `seen_by` field still rides the gossip wire format (written, read by nothing); removing it from `GossipFrame` is a tracked follow-up wire change. |

**Update:** **All five deep-pass HIGH residuals are now resolved.** D-2 (circuit isolation), T-1 (TOFU pinning + Contact), T-2 (signed invites) shipped as code changes. P-1 was verified closed by composition of HIGH-2 + T-1 + P-2 (analysis only). D-1 (ephemeral Noise) is implemented as an **opt-in** (`--ephemeral-noise-static`) with an honest composition note — leak #1 of §3.2's three closed by this flag; #2 and #3 close when the user composes the existing opts. Since then several MEDs have also landed: A0.3 (block sends to a key-changed pin), A1.2 (no-clearnet-leak guard), A1.3 (Full vanguards), and H-2 (gossip `seen_by` no longer trusted for drops). Remaining work is on the MEDs (P-3 delivery HOL, G-2 MLS committer authority, D-4 identity-derived onion) and the §3.2 architectural follow-ups (separated publish/subscribe connections, oblivious-recipient routing) that would let D-1 become default. The formal external audit (§8.2 #7) remains the single biggest open item.
