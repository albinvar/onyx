...
1. **Sealed-sender wrap on the daemon's hub path** (affects A2). **Sender path closed in T5.2.c**: the `SendBootstrap` API verb constructs a sealed-sender envelope (PQ-hybrid encapsulation, signed inner payload) and pushes it via the hub. The hub sees only the recipient's 16-byte introduction-inbox routing id and the opaque sealed body — exactly what §2 A2 promises. **What remains**: receive-side decode + delivery into the conversation registry (T5.2.d), MLS-Welcome variant (`v: mls/v1`) for post-compromise security on the hub path (T5.2.e), and TUI tier-distinction so users can tell `msg/v1` (PFS-only) apart from MLS-protected messages (T5.2.f). Current state on the wire: anyone can call `SendBootstrap` and a real sealed envelope reaches the hub, but on receipt the body is still discarded (logged only).
