...
**Update:** D-2, T-1, and T-2 are all now implemented (plus the D-3/T-3 follow-up close — see their rows + the CHANGELOG). The remaining HIGH residuals are **D-1** (ephemeral Noise keys — the §3.2 redesign) and **P-1** (conversation keying — verification-first).
