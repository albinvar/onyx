...
**Update:** D-2 (Tor circuit isolation) and T-1 (TOFU key pinning + key-change warning + `Contact` command) were implemented immediately after this pass (see their rows + the CHANGELOG). The remaining HIGH residuals are D-1 (ephemeral Noise keys), T-2 (invite auth), and P-1 (conversation keying — verification-first).
