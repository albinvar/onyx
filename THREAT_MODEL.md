...
1. **Sealed-sender wrap on the daemon's hub path** (affects A2). **First-contact `msg/v1` path closed end-to-end in T5.2.d**: sender wraps text in `BootstrapPayload::PlainMessage`, seals via PQ-hybrid encapsulation, ships to hub addressed to the recipient's introduction-inbox routing id. Recipient's `handle_hub_delivery` (in `onyxd::main`) tries `open_bootstrap` + `BootstrapPayload::from_cbor` on every inbound DELIVER; success paths register the sender as a hub-only peer in the conversation registry and push `EventMessage { via_hub: true }` so the TUI sees the message. **Decode failures are silent at debug level** to prevent log-spam attacks from a hostile hub. **What remains**: the `v: mls/v1` variant for true PCS over the hub path (T5.2.e) and the TUI tier-distinction rendering (T5.2.f — `via_hub` is already plumbed end-to-end through wire + ring buffer + History; only the visual indicator is missing).
...
15. **Hub does not validate publisher ownership of a routing id when storing a KeyPackage** (affects A2 indirectly — KP-directory integrity). T6.1 introduced `FRAME_KP_PUBLISH` with latest-wins semantics; an attacker connected to the hub can overwrite any other peer's published KP under that peer's routing id. **Recipient-side mitigation in place**: when fetching a KP for `target_fingerprint`, the recipient MUST verify that the KP's embedded Ed25519 signing key hashes to `target_fingerprint` before trusting it (the hub-side gap is recoverable end-to-end). Proper fix is a sign-challenge on publish — requires the hub to learn the publisher's Ed25519 key, which Noise XK doesn't surface; today the hub only knows the X25519 identity key. Tracked for future work.
