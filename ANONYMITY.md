...
A global passive adversary watching both your Tor entry guard and your peer's can correlate "Alice's daemon emitted a sealed envelope at 09:23:14.221" with "Bob's daemon emitted an `EventMessage` at 09:23:14.398." The hub knows this trivially because it sits in the middle.
  ...
  * **Stronger: constant-rate "high mode" (T-cover.const).** `--constant-rate-ms <MS>` on the daemon (or `onyxd`). The Poisson mode adds dummies *on top of* real traffic, so a real burst still rises above the noise floor and an adversary autocorrelating the rate can eventually pull it back out. Constant-rate removes the rate signal instead of masking it: every client→hub frame is funnelled through a fixed-slot pacer that emits **exactly one frame per slot** — a queued real frame if one is ready at the slot boundary, otherwise a `FRAME_PAD`. The observable upstream cadence is then *invariant* — one frame per slot whether you are actively chatting or idle — so the inter-frame timing distribution no longer separates "active" from "idle," which is the property the Poisson mode only approximates. Mutually exclusive with `--cover-traffic-mean-secs` (the pacer already fills idle slots; running both is incoherent and the daemon refuses it). Verified by the pacer unit tests (one-frame-per-slot, real-frames-first in FIFO order, idle-slots-emit-PAD, clean shutdown) and the e2e smoke `rooms_e2e_constant_rate_cover_does_not_break_flow` (a real room message still routes end-to-end through two paced daemons). **Honest limits specific to this mode:**
    * **Both directions can now be invariant (F1.1).** The daemon's `--constant-rate-ms` shapes client→hub (upstream); the **hub** binary now has a matching `--constant-rate-ms <MS>` (env `ONYX_HUB_CONSTANT_RATE_MS`) that shapes hub→client (downstream): a per-connection fixed-slot pacer emits exactly one frame per slot to each client — a queued real `FRAME_DELIVER` if one is waiting, otherwise a `FRAME_PAD`. Queued messages drained on SUBSCRIBE are fed through the same pacer (not flushed in a burst), so a backlog doesn't leak via timing. When both the daemon and the hub set `--constant-rate-ms`, the daemon↔hub channel is invariant-cadence in **both** directions. On the hub it **supersedes** `--cover-traffic-mean-secs` per connection (constant-rate already fills idle slots; running both just wastes bandwidth, so cover is disabled for that connection and a warning is logged). Verified by the hub unit test `constant_rate_paces_drain_and_pads_when_idle` (paced-not-bursted drain + idle PADs + per-slot cadence honored).
    ...
    * The wire-observer view is now **measured** (F1.2, `scripts/cover-traffic-measure.py`): constant-rate yields a metronomic 500 ms ± 1.2 ms cadence with uniform 272-byte frames (see #2 below). The remaining gap is the *real-Tor* end-to-end confirmation; the loopback measurement omits only Tor's content-independent transport jitter.
  * **What this does NOT do (be honest):**
    ...
    2. **Wire-observer measured (F1.2); real-Tor still operator-driven.** `scripts/cover-traffic-measure.py` interposes a transparent framing tap on the daemon↔hub socket and records the size + timing of every frame a wiretap would see. Measured (loopback, 2026-06-02): constant-rate at a 500 ms slot produces a **metronome** — inter-frame mean 500.0 ms, **stdev 1.2 ms (CV 0.002)**, uniform **272-byte** frames, both upstream and downstream; with shaping **off**, an idle link emits **zero** frames (silence trivially reveals idle). Poisson cover by contrast has stdev ~217 ms (CV 0.18) — a rate signal an autocorrelator can work on. Because every frame is equal-size ciphertext, a real message simply occupies a slot a PAD would have filled (same size, same cadence — the unit tests pin this), so active and idle are indistinguishable to the wire observer. **What loopback does *not* include: Tor's transport jitter.** That omission is safe for the *timing* claim (Tor jitter is a function of the circuit, not the frame contents, so it perturbs active and idle identically), but the full end-to-end confirmation over live circuits remains the operator drill (`scripts/real_tor_smoke.sh` + the same tap on the loopback SOCKS leg).
...
What Onyx v0 has, in layers of contribution:
  ...
  * **Private-by-default (D-1, the default).** As of the D-1 re-audit fix, the daemon runs **private by default** (`first_contact_reachable = false`): it uses a freshly-generated **ephemeral Noise static** AND a freshly-generated **ephemeral Ed25519 SUBSCRIBE-signing key** per hub connection, does **not** subscribe to `introduction_inbox(fp)`, and does **not** publish a KeyPackage. That closes **all four** channels above at once — the hub cannot link the connection to your long-term identity. Existing rooms still work (per-(room, epoch) session tokens carry no identity) and direct onion dials still work; the only thing you give up is being reachable for *first contact* via that hub. This supersedes the earlier `--ephemeral-noise-static` opt-in, which was theatre: it left the SUBSCRIBE proof carrying your fingerprint and still published a KP, so the hub re-linked you anyway. The adversarial test `rooms_e2e_private_mode_leaks_no_identity_to_hub` asserts the hub holds zero KeyPackages and doesn't know your intro inbox in this mode.
  * **`--first-contact-reachable` opt-in** — turn reachability back on: long-term Noise static + long-term SUBSCRIBE signing + `introduction_inbox(fp)` subscription + KeyPackage publish, so anyone with your fingerprint can reach you for first contact via this hub. The cost is the hub being able to link your *identity* to your activity. **As of F2.1a this cost is reduced**: a reachable daemon now runs the identity-bearing surface (KP publish + intro-inbox subscribe, long-term keys) on a **separate session/circuit** from its activity surface (room/DM session tokens + outbound, fresh ephemeral keys). So the hub still learns "you are reachable" (unavoidable when you opt in) but can **no longer link your fingerprint to *which rooms/DMs* you participate in**. The private default still spins up only the single ephemeral activity session. Trade-off in private mode: the hub's per-static-key rate limiter is per-connection (reconnect → fresh bucket); per-frame caps still bound resource use.
  * **Onion-service direct dials / connect-codes — the oblivious first-contact path.** The connect-code (`onyx://connect/v1?onion=…&id=…`, v0.1.16) is a direct onion dial that **bypasses the hub entirely** for first contact: no relay sees who is reaching whom. This is the recommended way to make first contact when you don't want a hub to learn it at all — strictly stronger than any hub-mediated first contact, because the structural first-contact leak (the deterministic `introduction_inbox(fp)` a stranger and the hub can both compute) simply doesn't apply when there is no hub in the path. Hub intro-inbox first-contact remains the explicit reachability tradeoff for when you *do* want to be reachable by fingerprint alone.

What would actually close all three leaks together: ephemeral Noise keys per session (**done, D-1 — now the default**), separated identity/activity connections (**done, F2.1a — reachable mode no longer links identity to room set**), and oblivious-recipient routing for first-contact (deferred; PIR/ORAM-grade — `ROTATION.md` §6). For first-contact privacy *today*, use the connect-code path (above), which removes the hub from first contact entirely. The established-rooms profile already has full leak closure via D-1 + session tokens.
...
A long-running onion service repeatedly builds circuits; an adversary who can correlate those circuits over time can mount a *guard-discovery* attack to learn the service's entry guard, then attack/coerce it to deanonymize the host. Tor's mitigation is **vanguards**: slow-rotating L2 (and, for HS, L3) guard layers that drastically raise the time/cost of discovering the guard.
  ...
  * **Scope (honest):**
    ...
    2. **Both modes publish a v3 HS, so both have a guard-discovery surface** — A1.3 protects both. (Correcting an earlier overclaim that the private default published no HS: the daemon's accept loop publishes the onion service unconditionally, because direct onion dials are a documented feature of private mode — D-1 removes *hub* linkage, ephemeral Noise + no intro-inbox + no KP, **not** the onion service. The onion key is random, not identity-derived (A6.4), so publishing it leaks no fingerprint; it is a *presence* surface, not an identity one.) What does differ: a private-default node still discloses far less elsewhere (no hub-linkable long-term key, no KP directory entry), but its HS circuits get the same Full-vanguard protection as a reachable node's. **Open product question (tracked):** whether the private default *should* gate the HS publish — that would make the node hub-only / un-dialable for direct first contact, a reachability trade we have not decided.
